...
function BlockedTask () {
  const [todos, setTodos] = useState(() => {
    const savedTodos = localStorage.getItem('btodos')
    if (savedTodos) {
  ...
  return (
    <div className='mx-4'>
      ...
      <div className='flex flex-col w-full'>
        <label className='text-red-800 bg-red-300/80  font-bold my-4 w-fit rounded-lg px-4 py-2'>
          BackLog
        </label>
        <input
        ...
        />
        <button
          className='bg-sky-300 font-bold text-neutral-800 px-4 py-2 rounded-lg mt-4 '
          type='submit'
          onClick={handleAdd}
        >
          Add
      ...
      <div className='bg-slate-800 min-h-96 w-auto lg:w-72  py-2 px-4 my-4 rounded-lg '>
        <div className=' '>
          <ul className='  '>
            {todos.map(todo => (
              <div className='flex p-4 bg-red-400 rounded-lg text-gray-800 font-semibold flex-row my-4 justify-between'>
