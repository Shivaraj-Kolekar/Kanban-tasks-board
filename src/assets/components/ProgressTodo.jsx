...
function ProgressTodo () {
  const [todos, setTodos] = useState(() => {
    const savedTodos = localStorage.getItem('ptodos')
    if (savedTodos) {
  ...
  useEffect(() => {
    localStorage.setItem('ptodos', JSON.stringify(todos))
  }, [todos])
  ...
  return (
    <div className='mx-4'>
      <div className='flex flex-col w-full'>
        <label className='font-bold text-yellow-800 bg-yellow-300/80 my-4 w-fit rounded-lg px-4 py-2'>
          In Progress
        </label>
        <input
        ...
        />
        <button
          className='bg-sky-300 px-4 py-2 font-bold text-neutral-800 rounded-lg mt-4 '
          type='submit'
          onClick={handleAdd}
        >
          Add
      ...
      <div className='bg-slate-800 min-h-96 w-auto lg:w-72 py-2 px-4 my-4 rounded-lg '>
        <div className=' '>
          <ul className=''>
            {todos.map(todo => (
              <div className='flex h-auto py-2 px-4 bg-yellow-400 text-gray-800 font-semibold rounded-lg flex-row  my-4 justify-between'>
                <li className='m-2 flex flex-wrap' key={todo.id}>
                  {todo.text}
                </li>
                <button
