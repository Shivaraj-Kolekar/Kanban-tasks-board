...
function CompletedTask () {
  const [todos, setTodos] = useState(() => {
    const savedTodos = localStorage.getItem('ctodos')
    if (savedTodos) {
  ...
  useEffect(() => {
    localStorage.setItem('ctodos', JSON.stringify(todos))
  }, [todos])
  ...
  return (
    <div className='mx-4'>
      ...
      <div className='flex flex-col w-full'>
        <label className='text-teal-800 bg-teal-300/80  font-bold my-4 w-fit rounded-lg px-4 py-2'>
          Completed
        </label>
      ...
      <div className='bg-slate-800 min-h-96 w-auto lg:w-72 py-2 px-4 my-4 rounded-lg '>
        <div className=' '>
          <ul>
            {todos.map(todo => (
              <div className='flex p-4 bg-teal-400  text-gray-800 font-semibold rounded-lg flex-row my-4 justify-between'>
